...
const BrokerDetails = () => {
  ...
  const broker = location.state; // Access the broker data passed from the table

  if (!broker) {
    return <div className="text-red-500">Broker not found.</div>; // Handle case where no broker is found
  }
    ...
    <div className="w-full h-full bg-[#F5F7F7] p-10 overflow-auto">
      <div className="grid grid-cols-2 gap-10">
        <div className="space-y-6">
          <div className="bg-white rounded-[18px] p-6 shadow-lg flex items-center gap-6">
            <img
              src="https://i.pravatar.cc/100?img=5" // Placeholder profile image
              alt="profile"
              className="w-[100px] h-[100px] rounded-full"
            />
            <div>
              <h2 className="text-[22px] font-semibold text-black">{broker.accountHandlerName}</h2>
          ...
          <div className="bg-white rounded-[18px] p-7 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="flex flex-col bg-[#FAFAFA] p-4 rounded-lg">
                ...
                <span className="text-[16px] font-medium text-black">{broker.email}</span>
              </div>
              <div className="flex flex-col bg-[#FAFAFA] p-4 rounded-lg">
                <span className="text-[14px] text-[#9E9E9E]">Tax Identification Number</span>
                <span className="text-[16px] font-medium text-black">{broker.companyTaxIdenfication}</span>
              </div>
        ...
        {/* Additional details and sections can go here */}
      </div>
    </div>
  );
};
